...
These notes describe a review workflow for finding stored and reflected XSS across an application, including user-facing flows, administrator-only areas, and out-of-band inputs.

## When it applies

Use this checklist when you are mapping an application for possible XSS entry points and trying to confirm where controlled data is stored, reflected, or replayed.
...
The original notes focus on breadth: revisit every place where user-controlled data may surface, including multi-step workflows and administrator-only views.
